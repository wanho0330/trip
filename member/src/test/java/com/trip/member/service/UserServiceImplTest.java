...
class UserServiceImplTest {
    ...
    @Mock
    private MemberKafkaProducer kafkaProducer;
    ...
    void createUser_Success() {
        ...
        doNothing().when(kafkaProducer).send(any(KafkaEvent.class));
        ...
        //TODO : Kafka Event Generic 변경으로 인한 오류. 추후 ㅇ처리 필요
//        ArgumentCaptor<KafkaEvent> kafkaEventCaptor = ArgumentCaptor.forClass(KafkaEvent.class);
//        verify(kafkaProducer, times(1)).send(kafkaEventCaptor.capture());
//
...
//        assertEquals(email, capturedEvent.getAfterUser().getEmail());
    ...
    void createUser_EmailAlreadyExist_ThrowException() {
        ...
        verify(kafkaProducer, never()).send(any(KafkaEvent.class));
    }
    ...
    void getAllUsers_Success() {
        ...
        verify(kafkaProducer, times(1)).send(Mockito.any(KafkaEvent.class));
    }
    ...
    void getAllUsers_CursorNotFound_ThrowsException() {
        ...
        verify(kafkaProducer, never()).send(Mockito.any(KafkaEvent.class));
    }
